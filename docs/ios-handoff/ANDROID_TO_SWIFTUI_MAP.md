...
| [FormHomeScene.kt](../../app/src/main/java/com/thevaguebox/probabilistictictactoe/ui/components/FormHomeScene.kt) | Stationary code-drawn bag/arrows/board; central production X/O pieces alternate every 1400 ms within fixed 44 dp geometry | Reuse native production pieces and semantic colors; cancelable visible/active-scene timeline, prescribed pulse/crossfade, no scale with Reduce Motion, instant swaps if animations disabled; one stable explanatory description, no repeated VoiceOver announcements; no raster assets or RNG |
...
**Android game-state fact:** `collectAsStateWithLifecycle()` governs UI collection; `PresentationClock` is composition-scoped; `viewModelScope` governs AI. There is no explicit game-background checkpoint policy, exact remaining-time snapshot, or resume-specific game transition in the checked-in code. Do not claim Android fully pauses every presentation or AI search when backgrounded. A pending delay/search can complete according to the lifecycle and coroutine state, while UI collection may be stopped.

**Separate Home illustration policy:** its local X/O loop runs only while visible and lifecycle RESUMED after stored preferences load; inactive/offscreen/disposed stops the loop. It restarts from X, without persisted animation phase, random draws, game callbacks or feedback events. This is a deliberately scoped visual lifecycle guard, not a change to game-state/AI scheduling or wordmark entrance consumption. Port with active-scene/visibility cancellation and the exact [motion contract](motion-spec.json).
