...
The iOS workflow reports independent native-package/shared-fixture, hosted application integration, player-driven XCUITest, unsigned Debug and Release for both simulator and generic iOS device SDKs, and governed snapshot gates. `xcodebuild` runs with signing disabled. The platform-aware Release artifact gate checks bundle/version identity, the actual Mach-O platform and iOS 17 deployment target in every slice, iPhone/iPad support, compiled icon/launch resources, exact canonical fonts/OFL and policy bytes, absent signing material/test bundles and absence of development-only launch switches/type names. A generic device SDK build is compile-time evidence only; it does not claim a physical-device run. Snapshot comparison never updates approved baselines. Test result bundles, snapshot candidates/comparisons and failure build logs remain reviewable Actions artifacts, with explicit job summaries.
