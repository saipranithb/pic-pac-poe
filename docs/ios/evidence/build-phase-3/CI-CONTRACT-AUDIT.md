...
| Unsigned native Release builds | PASS on `03d44f794ef98e66ebeb63cd467313ece2331b91`: simulator 15.753 seconds, device SDK 13.519 seconds; strengthened actual-binary checks pass with zero coverage instrumentation |
| Unsigned native Debug builds | PASS on the same source: simulator 16.318 seconds, device SDK 10.341 seconds; all artifacts unsigned and minimum iOS 17.0. Device SDK compilation is not device execution |
...
The earlier unsigned builds compiled with stable source and passed their then-current identity/resource gate, but independent binary inspection found LLVM coverage sections. Their full package is preserved locally at `/tmp/picpac-phase3-unsigned-builds-ad1-before-coverage`; compact [finding, source provenance, compile flags and binary hashes](defects/release-coverage/finding.json) are retained here. The strengthened gate rejects that actual earlier binary. The corrected final four-cell build matrix is retained in [unsigned-builds-final-20260925/manifest.json](unsigned-builds-final-20260925/manifest.json), including complete app-file hashes and exact commands. Both rebuilt Release binaries pass the strengthened gate: no LLVM coverage/profile sections or instrumented compiler commands remain in the app or linked packages. Every simulator Mach-O slice (arm64 and x86_64) and device SDK slice (arm64) declares the correct platform and minimum iOS 17.0. All 13 retained build-evidence files pass their checksum inventory. The only compiler warning is the expected App Intents metadata extraction skip because this app does not depend on AppIntents.framework.
